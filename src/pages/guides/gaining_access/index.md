...
Workfront today does not support the Admin Console "Developer" role, so the only way to access the developer console is to be a full IMS Org admin. This is NOT the same as a Workfront product admin, but someone who manages the whole org and all products under it. You will need to work with the system admin of the Org to help configure the "Workfront API" credentials (following the instruction below) or ask to become a full system admin. Making someone an Org system admin can only be done by other IMS Org system admins. 

![image](images/ims-org-admin.png "Console")
...
![image](images/make_admin.png "Console")
